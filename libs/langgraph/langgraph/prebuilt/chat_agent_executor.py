...
class AgentStateWithStructuredOutput(AgentState):
    """Extended agent state that includes structured response for output parsing."""
...
def parse_structured_response(
    ...
    try:
        ...
        raise ValueError(
            f"Failed to parse AI message content into {response_format.__name__}: {str(e)}"
        ) from e
    ...
    *,
    ...
    def should_continue(state: AgentState) -> Literal["tools", "respond", "__end__"]:
        ...
        if not isinstance(last_message, AIMessage) or not last_message.tool_calls:
            # If response_format is provided, route to respond node for structured parsing

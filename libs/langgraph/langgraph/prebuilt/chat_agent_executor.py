    ...
    *,
    ...
    Args:
        ...
        response_format: An optional Pydantic model class to use for structured output.
            When provided, the agent will return structured data in the specified format
            in addition to the regular conversational messages. The structured output will
            be available in the final state under the "structured_response" key.
    ...
    def should_continue(state: AgentState) -> Literal["tools", "__end__"]:
        ...
        # If structured output is requested and we have a structured response, we're done
        if response_format is not None and "structured_response" in state and state["structured_response"] is not None:
            return "__end__"
